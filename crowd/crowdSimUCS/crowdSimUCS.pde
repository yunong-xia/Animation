...
float sfk = 15000; // seperation force K
...
int[] USC(int start, int goal) {  // return an optimal path according to start and goal
  // first, initialize d[]
  //A* variables
  ...
  while(frontier.size() != 0) {
    int current = dequeue(frontier, d); // current vertex index
...
void setup() {
  initObstacles();
  PRM(); 
  p = USC(globalGoal, globalStart);  // here we use backward AStar. The AStar won't stop even if it finds the "goal" we set. So the p[i] is the next point an agent want to reach next. 
