...
void initObstacles() {
  for(int i = 0; i < numObst; i ++) {
    obst_x[i] = -3*obstRad+(2*i)*obstRad;
    obst_y[i] = 0;
  }
...
void computeForces() {
  for(int i = 0; i < numAgent; i++) {
    ...
    for(int j = 0; j < numAgent; j++) {  // neighbors
      ...
      float awayDistance = sqrt(awaySx*awaySx + awaySy*awaySy)-1.1*agentRad;
    ...
    for(int j = 0; j < numObst; j++) {   // obstacles
      ...
      if (vx[i]*(-awayObstSx) + vy[i]*(-awayObstSy) < 0)
        continue;
      float awayObstDistance = sqrt(awayObstSx*awayObstSx + awayObstSy*awayObstSy) - 1.2*(obstRad + agentRad);
